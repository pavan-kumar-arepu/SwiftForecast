...
struct CurrentWeatherView: View {
    ...
    /// ErrorMessage to show whenn city search returned Error
    ...
    private func fetchWeatherForCity() {
        ...
        geocoder.geocodeAddressString(selectedCity) { placemarks, error in
            guard let placemark = placemarks?.first, let location = placemark.location else {
                errorMessage = AlertMessage(message: "Unable to geocode city name")
                return
            }
...
/// Represents an error message to be displayed in an alert.
